...
describe("FileSystem", () => {
  ...
  describe("readFile and writeFile", () => {
    ...
    it("should create parent directories when writing", async () => {
      ...
      expect(readContent).toBe(content);
      expect(
        await FileSystem.exists(path.join(tempDir, "nested", "deep")),
      ).toBe(true);
    });
  });
  ...
  describe("copy", () => {
    ...
    it("should copy directory recursively", async () => {
      ...
      expect(await FileSystem.exists(path.join(destDir, "file1.txt"))).toBe(
        true,
      );
      expect(
        await FileSystem.exists(path.join(destDir, "nested", "file2.txt")),
      ).toBe(true);
    });
  });
