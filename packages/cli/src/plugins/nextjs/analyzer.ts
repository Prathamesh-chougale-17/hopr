...
import { fileExists, readJSON, findFiles, normalizePath, readFile } from '../../utils/index.js'
 ...
 */
export async function analyzeNextJsProject(projectPath: string): Promise<ProjectStructure> {
  const useSrc = await usesSrcDirectory(projectPath)
  const appDir = useSrc ? path.join(projectPath, 'src', 'app') : path.join(projectPath, 'app')
  ...
  const hasTypescript = await fileExists(path.join(projectPath, 'tsconfig.json'))

  const hasMiddleware = await fileExists(path.join(projectPath, 'middleware.ts')) ||
 ...
 */
async function findRoutes(appDir: string, projectRoot: string): Promise<RouteInfo[]> {
